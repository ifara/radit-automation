...
setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
         ...
         //Aceptar/Cancelar
       connect(BtnAceptar,SIGNAL(clicked()),this, SLOT(ClickAceptar())); //Aceptar
       connect(BtnCancelar,SIGNAL(clicked()),this, SLOT(close())); //Cancelar

       connect(ComboEvento,SIGNAL(activated(int)),this, SLOT(ClickTipo(int))); //Evento tipo
        ...
        connect(TimeHoraInicio, SIGNAL(editingFinished()), this, SLOT( HoratoCheckBox()));
         ...
         connect(BtnUrl,SIGNAL(clicked()),this, SLOT(ClickUrl())); //Aceptar
         connect(BtnAddRadio,SIGNAL(clicked()),this, SLOT(AddRadio())); //Aceptar
...
void AddEventos::OtrasHoras(){
      ...
      for(int i=0;i < 24;i++){
          ...
          if(hora[i]->isChecked()){Cuantos++;}  //averiguamos cuantos hay


         }

      if(Cuantos==1){RadioUnaVez->setChecked(true);}  // SI SOLO es una hora activa radio una vez
      if(Cuantos==24){RadioCadaHora->setChecked(true);}  // si son todas activa radio cada hora
...
void AddEventos::ClickAceptar(){
    ...
    if(Url.isEmpty()){
     ...
     if(ModoEdit){
         UpdateEvento();  //actualizamos
         this->close();
         return;
       }

    NuevoEvento();     //añadimos nuevo
...
void AddEventos::ClickTipo(int Tipo){
    ...
    switch (Tipo) {
        ...
        case 0:
         this->FrameTipoFichero->setVisible(true);  //fichero
        ...
        case 2:
             this->FrameTipoFichero->setVisible(false); //hora
...
QString AddEventos::HorasToString(){
    ...
    if(CuantasHoras==24)                   //todos loa dias
       return("Cada hora");
